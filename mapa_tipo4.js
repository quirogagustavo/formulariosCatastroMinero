...
fetch("consulta_gra_servidumbre_linea.php")
    .then(res => res.json())
    .then(data => {
      capaMensuraArea = L.geoJSON(data, {
      ...
      overlayMaps["Servidumbre Linea"] = capaMensuraArea;
      actualizarControlCapas();
    });

    
   fetch("consulta_gra_servidumbre_poligono.php")
    .then(res => res.json())
    .then(data => {
      capaMensuraPertenencias = L.geoJSON(data, {
        onEachFeature: function (feature, layer) {
          layer.bindPopup(
            `<strong>Expte:</strong> ${feature.properties.expediente}<br>` +
            `<strong>Denominación:</strong> ${feature.properties.denominacion}<br>` 
      ...
      overlayMaps["Servidumbre Polígono"] = capaMensuraPertenencias;
      actualizarControlCapas();
    });   
